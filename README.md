...
Then save the answer, so it can be consulted later.
...
>
> Then, we query through de database, and check if there's already collected cities with this id (user_id is not unique, but it can only call API once).
...
## The `GET` method
Responsible for receiving a user_id, and query through the database for checking if the collecting method is completed. If not, we return the percentage of the process.
...
```
> This method queries in database with the received user id, and at first checks if it has rows. If not, we return a False statement.
> ```
...
- .env file containing the private access-token to Open Weather API
...
5.Back at root directory install every dependencies  
...
After starting the application, with ou without docker, it should be running at your `localhost`
...
# FYI
`.env`: Configuration file that is not on the repository but is needed to run the application. Because it has the access token to make request to Open Weather API.  
